...
#import "JsBridgeResource.h"

#define JsBridge_Export_Func_Config_Prefix_Format @"JsBridge_Export_Func_Config_Prefix_%@"
#define JsBridge_Export_Func_Config_Internal(selArg, ...) \
- (NSDictionary *)JsBridge_Export_Func_Config_Prefix_ ## selArg { \
...
#define JsBridge_Export_Func(sel, ...) JsBridge_Export_Func_Config_Internal(sel, ## __VA_ARGS__)

@protocol JsBridgeApiProtocol <NSObject>
@required
/**  方法说明
 如： js中要使用api ---->  myApi.request({})      jsBridge_iosApiPrefix = @'js_"
 则：- (NSString *)jsBridge_jsApiPrefix{return @"myApi"}
 对应的原生方法实现 - (void)js_request:(JsBridgeApiArgItem *)arg <#xxx:(JsBridgeApiArgItem *)xxx#>{}
 
 如： js中要使用api ---->  myApi.requireModule('xxx').play({})
 则：- (NSArray <id<JsBridgeApiProtocol>> *)jsBridge_apiModules返回的实例对象要服从JsBridgeApiProtocol协议
 */
//js api方法名前缀  如：myApi
...
/**
 ⚠️⚠️⚠️添加API步骤：
 在服从协议JsBridgeApiProtocol的类中实现方法即可：
 异步方法
   - (void)js_<#functionName#>{}
   - (void)js_<#functionName#>:(JsBridgeApiArgItem *)arg{}
   - (void)js_<#functionName#>:(JsBridgeApiArgItem *)arg <#xxx:(JsBridgeApiArgItem *)xxx#>{}

 同步方法
   //返回JS类型Object
   - (NSDictionary *)js_<#functionName#>Sync{}
   - (NSDictionary *)js_<#functionName#>Sync:(JsBridgeApiArgItem *)arg <#xxx:(JsBridgeApiArgItem *)xxx#>{}

   //返回JS类型Array
   - (NSArray *)js_<#functionName#>Sync{}
   - (NSArray *)js_<#functionName#>Sync:(JsBridgeApiArgItem *)arg <#xxx:(JsBridgeApiArgItem *)xxx#>{}

   //返回JS类型String
   - (NSString *)js_<#functionName#>Sync{}
   - (NSString *)js_<#functionName#>Sync:(JsBridgeApiArgItem *)arg <#xxx:(JsBridgeApiArgItem *)xxx#>{}

   //返回JS类型Number
   - (NSNumber *)js_<#functionName#>Sync{}
   - (NSNumber *)js_<#functionName#>Sync:(JsBridgeApiArgItem *)arg <#xxx:(JsBridgeApiArgItem *)xxx#>{}
...
/* 通信各个阶段数据类型
 ...
 👉同步通信返回值： [原生->js]  const res = myApi.testSync(xxx);
     A：web各阶段数据类型
         1、JsBridgeApiProtocol同步函数返回数据(默认返回nil)
 ...
 👉[异步/同步]通信： [js->原生]  myApi.test(xxx)  xxx传参类型
     A：web各阶段数据类型 (JsBridgeApiProtocol异步函数返回值: 丢弃不予处理，web端 const res = myApi.getTest111(Undefined); 执行后  res为[object Undefined]类型)
         ...
         6、JsBridgeApiProtocol异步函数接收数据: JsBridgeApiArgItem.jsData = ((!jsData || [jsData isEqual:[NSNull null]]) ? nil : jsData)
     ...
     B、JSCore各阶段数据类型 (JsBridgeApiProtocol异步函数返回值: 参见JSCore同步通信)
         1、api调用者传参数据:  const res = myApi.getTest111(Undefined);
         2、原生接收JSValue参数类型: JSValue
         3、原生接收JSValue参数转数据: [JSValue toObject]
         4、JsBridgeApiProtocol异步函数接收数据: JsBridgeApiArgItem.jsData = ((!jsData || [jsData isEqual:[NSNull null]]) ? nil : jsData)
 ...
 👉[异步/同步]通信回调： [原生->js]  myApi.test({success: function(e){}})  回调e参数类型
     A、web回调各阶段数据类型
         1、JsBridgeApiProtocol异步函数回调传参: JsBridgeApiCallJsItem.call()
         2、包装数据成数组:
 ...
 👉[异步/同步]通信回调后，js处理后返回原生的数据： [js->原生]  myApi.test({success: function(e){return xxx}})  xxx参数类型
     A、web各阶段数据类型
         ...
         3、原生数据处理: JsBridgeApiCallJsResItem.result = ((!result || [result isEqual:[NSNull null]]) ? nil : result)
         4、JsBridgeApiProtocol异步函数中获取到的js返回数据 jsResItem.result:
